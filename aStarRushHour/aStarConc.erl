...
-export([solve/4]).
-export([aStarStep/4, slave/6, loop/3]). % for spawning
    ...
    next_state ->
      case ets:first(openSet) of
        {Value, State} ->
          ...
          spawn(?MODULE, slave, [self(), State, State, BoundX, BoundY, {GoalX, GoalY}]),
          aStarStep(Pid, BoundX, BoundY, {GoalX, GoalY});
        '$end_of_table' ->
          self() ! next_state,
          aStarStep(Pid, BoundX, BoundY, {GoalX, GoalY})
      end
  after 20 -> Pid ! {answer, -1} 
  end.

loop(BoundX, BoundY, {GoalX, GoalY}) ->
  receive 
    {work, {Car, State, Step}} ->
...
slave(Pid, State, State, BoundX, BoundY, {GoalX, GoalY}) ->
  ...
  case H of
    ...
    _ -> 
      ...
      Slaves = [spawn(?MODULE, loop, [BoundX, BoundY, {GoalX, GoalY}]) || _N <- State],
      [Slave ! {work, {Car, State, G+1}} || {Slave, Car} <- lists:zip(Slaves, State)]
  end.

%% Main Function
%% try-catch used for continuous calls in ets without problems
